...
def kapa(
        ...
        clone_multiplier
        # Rate to clone an antibody relative to its affinity (how good it is)
):
    ...
    num_kill_antibodies = min(num_kill_antibodies,
                              len(antibodies) - len(antigens))
    for g in range(generations):
        if early_termination or all(
                antibody.memfinity == 0 for antibody in memory_antibodies):
            if not early_termination:
                print('Terminating early at generation {0}'.format(g))
                early_termination = True
            for antigen_key in range(len(antigens)):
                affinity_graph.append(
                    {'key': antigen_key, 'generation': g, 'affinity': 0})
            continue
        print(g)
        """
        ...
        for antigen_key, antigen in enumerate(antigens):
            ...
            """
            antibodies.sort(
                key=lambda antibody: antibody.affinity)  # Sorts by affinity
            ...
            for antibody in antibodies[:num_clone_antibodies]:
                ...
                """
                affinity_rank += 1
                antibody_clones = clone_antibody(antibody, clone_multiplier,
                                                 num_antibodies, affinity_rank)
                """
            ...
            if best_antibody.affinity < memory_antibodies[
                antigen_key].memfinity:
            ...
            affinity_graph.append({'key': antigen_key, 'generation': g,
                                   'affinity': memory_antibodies[
                                       antigen_key].memfinity})
            """
            ...
            if num_kill_antibodies > 0:
                ...
                for i in range(len(antibodies) - 1, -1, -1):
                    ...
                    if antibodies[i] not in memory_antibodies:
                        num_to_kill -= 1
                        del antibodies[i]
                antibodies = antibodies + [Antibody() for _ in
                                           range(num_kill_antibodies)]
